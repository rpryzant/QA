...
# Inputs
parser.add_argument('--input_scene_file', default='wikimovies.short.json',
        ...
        if next_node['type'] in special_nodes:
            if next_node['type'].startswith('relate_filter'):
                ...
                if next_node['type'] == 'filter':
                    # Remove null filter
                    filter_length = len(filter_options.keys()[0])
                    filter_options.pop(tuple(None for _ in range(filter_length)), None)
            ...
            for filter_key in filter_option_keys:
                new_nodes = []
                ...
                for param_name in filter_side_inputs:
                    param_type = param_name_to_type[param_name]
                    param_idx = ATTR_KEYS.index(param_type)
                    param_val = filter_key[param_idx]
